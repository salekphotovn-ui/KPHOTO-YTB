...
- Đã đồng bộ `BBDown.exe` v1.6.3 và `BBDown.data` từ `D:\New folder\AUTO\BBDown`; V3 gọi lệnh tối giản giống batch tải nhanh, thêm `--force-http` và `--multi-thread`, không ép `--select-page`/file pattern.
